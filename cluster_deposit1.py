...
db = DBSCAN(eps=30, min_samples=5, metric='precomputed')  # using "precomputed" 
y_db = db.fit_predict(distance_matrix)

df1['cluster'] = y_db

plt.scatter(df1['longitude'], df1['latitude'], c=df1['cluster'])
plt.show()
...
first_cluster.to_csv('first_cluster.csv', encoding='utf-8', index=False)
second_cluster.to_csv('second_cluster.csv', encoding='utf-8', index=False)
three_cluster.to_csv('three_cluster.csv', encoding='utf-8', index=False)
